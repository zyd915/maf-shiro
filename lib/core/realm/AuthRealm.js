...
class AuthRealm extends CacheRealm_1.CacheRealm {
    ...
    getAuthInfo(token) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // let that = this;
            // let promise = new Promise<AuthInfo>(function (resolve, reject) {
            //     let authc = that.getAuthenticationInfo(token);
            //     let authInfo: AuthInfo, authcInfo: AuthenticationInfo = null, authzInfo: AuthorizationInfo = null;
            //     let authcError = false, authc_error_msg: string = '';
            //     authc.then(function (authenticationInfo: AuthenticationInfo) {
            //         authcInfo = authenticationInfo;
            //     }, function (err_msg) {
            //         authcError = true;
            //         authc_error_msg = err_msg;
            //     });
            //     if(authcError){
            //         reject(authc_error_msg);
            //     }
            //     let authz = that.getAuthorizationInfo(token);
            //     authz.then(function (authorizationInfo: AuthorizationInfo) {
            //         authzInfo = authorizationInfo;
            //     }, function (err_msg) {
            //         authzInfo = null;
            //     });
            //     authInfo = new AuthInfo(authcInfo, authzInfo);
            //     resolve(authInfo);
            // });
            //
            // return promise;
